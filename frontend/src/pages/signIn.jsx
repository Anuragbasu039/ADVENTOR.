...
// const API_BASE_URL = window.location.hostname === 'localhost' ? 'http://localhost:8000' : 'https://adventor-liard.vercel.app';
const API_BASE_URL =
...
const SignInPage = () => {
    ...
    const handleSignIn = async (e) => {
        ...
        try {
            const response = await axios.post(`${API_BASE_URL}/api/auth/signin`, { email, password }, { withCredentials: true });
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen">
            <div className="w-full h-full md:w-1/2 bg-cover bg-center relative rounded-full" style={{
            ...
            <div className="w-full md:w-1/2 flex items-center justify-center bg-white p-4 md:p-8">
                <div className="max-w-md w-full">
                    ...
                    <form onSubmit={handleSignIn}>
                        ...
                        <button type="submit" className="w-full bg-teal-600 text-white py-2 rounded-md hover:bg-blue-600 transition duration-200" disabled={!isFormValid || isLoading}>{isLoading ? 'Signing in...' : 'LOGIN'}</button>
                    </form>
